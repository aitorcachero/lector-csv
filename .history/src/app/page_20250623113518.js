...
// En la función handleFileUpload, reemplazar el comentario con:
else if (isDBC) {
  try {
    // Crear un archivo temporal para DBFFile
    const tempFile = new File([buffer], fileName, { type: 'application/octet-stream' });

    // Usar DBFFile para leer el archivo
    const dbf = await DBFFile.open(tempFile.path || fileName, { readMode: 'loose' });
